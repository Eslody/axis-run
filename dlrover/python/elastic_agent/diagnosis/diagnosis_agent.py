...
from dlrover.python.common.constants import NodeEnv, TrainingExceptionLevel
...
class DiagnosisAgent(Singleton, DiagnosisManager):
    def __init__(
        ...
        self._load_extension_diagnosticians()
    ...
    def _load_extension_diagnosticians(self):
        """通过环境变量 DLROVER_EXTENSION_DIAGNOSTICIAN 动态加载外部 Diagnostician。

        环境变量格式为逗号分隔的 "module_path::ClassName"。每个类必须继承自
        ``dlrover.python.diagnosis.common.diagnostician.Diagnostician``，构造函数
        支持无参调用（可从环境变量读取配置）。加载失败的项仅记录警告，不中断 agent。
        """
        ext = env_utils.get_env(NodeEnv.DLROVER_EXTENSION_DIAGNOSTICIAN)
        if not ext:
            return
        ...
        for item in ext.split(","):
            ...
            if not match:
                logger.warning(
                    f"Invalid extension diagnostician config: {item}, "
            ...
            try:
                ...
                logger.warning(
                    f"Failed to import extension diagnostician "
                    f"{class_name} from {module_path}: {e}"
                )
                continue

            if not issubclass(cls, Diagnostician):
                logger.warning(
                    f"{class_name} must inherit from Diagnostician."
                )
                continue
            ...
            # 默认 30 秒周期，使用类名作为 diagnostician name，
            # 避免与内置 diagnostician 名称冲突。
            self.register_diagnostician(class_name, instance, 30)
            logger.info(
                f"Loaded extension diagnostician: {module_path}::{class_name}"
            )
